...
#ifndef AUDIO_FUNC_DBG
#define AUDIO_FUNC_DBG(fmt, ...) \
  printf("%s ", __func__);       \
...
static unsigned char* _a_get_file(const char* file, uint32_t* size_ptr) {
  ...
  if (!data) {
    printf("You've got some serious problems buck-o\n");
    fclose(f);
    return 0;
  }
  ...
  if (read_amount != data_len) {
    printf("You've been bamboozled by this file man.\n");
    free(data);
    return 0;
  }
...
static uint8_t _a_layer_add(a_layer* layer, uint32_t id, uint8_t is_sfx) {
  if (is_sfx) {
    if (layer->sfx_count == layer->sfx_capacity) {
      AUDIO_FUNC_DBG("no free sfx slots in layer\n");
      return 0;
    }

    for (uint16_t i = 0; i < layer->sfx_capacity; ++i) {
      if (layer->sfx[i] == id) {
        AUDIO_FUNC_DBG("sfx id [%i] already in layer\n", id);
        return 0;
      }
    ...
    if (layer->song_count == layer->song_capacity) {
      AUDIO_FUNC_DBG("no free song slots in layer\n");
      return 0;
    }

    for (uint16_t i = 0; i < layer->song_capacity; ++i) {
      if (layer->songs[i] == id) {
        AUDIO_FUNC_DBG("song already in layer\n");
        return 0;
      }
...
void a_efx_info(a_ctx* ctx) {
  if (alcIsExtensionPresent(ctx->device, "ALC_EXT_EFX") == AL_FALSE) {
    AUDIO_FUNC_DBG("No ALC_EXT_EFX.\n");
  } else {
    AUDIO_FUNC_DBG("ALC_EXT_EFX Present.\n");
  }
...
#endif
  AUDIO_FUNC_DBG("MAX_AUXILIARY_SENDS: %i\n", s_sends);
}
...
a_ctx* a_ctx_create(const char* device, uint8_t layers, uint16_t max_sfx,
  ...
  if (!ctx) {
    AUDIO_FUNC_DBG("unable to malloc initial space for context.\n");
    return 0;
  }
  ...
  if (!alcMakeContextCurrent(context)) {
    AUDIO_FUNC_DBG("Error creating OpenAL Context\n");
    free(ctx);
    return 0;
  }
  ...
  if (ctx->use_fx) {
    ...
    if (!ctx->fx_per_source) {
      AUDIO_FUNC_DBG("0 effects allowed per source, disabling effects.\n");
      ctx->use_fx = 0;
    }
  }
...
#undef LOAD_PROC
    ...
    if (max_fx) {
      ...
      if (!ctx->fx_slots) {
        AUDIO_FUNC_DBG("unable to allocate %i fx slots\n", ctx->fx_capacity);
...
uint8_t a_ctx_destroy(a_ctx* ctx) {
  if (!ctx) {
    AUDIO_FUNC_DBG("no context passed to destroy.\n");
    return 0;
  }
...
uint16_t a_sfx_play(a_ctx* ctx, uint16_t layer, uint16_t buf_id, a_req* req) {
  if (ctx->sfx_count == ctx->sfx_capacity) {
    AUDIO_FUNC_DBG("no free sfx slots.\n");
    return 0;
  }
  ...
  if (layer) {
    ...
    if (_layer->sfx_count == _layer->sfx_capacity) {
      AUDIO_FUNC_DBG("no free slots in layer\n");
      return 0;
    }
  }
  ...
  if (!buf) {
    AUDIO_FUNC_DBG("unable to find %i\n", buf_id);
    return 0;
  }
  ...
  if (!slot) {
    AUDIO_FUNC_DBG("No slots open, really.\n");
    return 0;
  }
...
uint16_t a_song_create(a_ctx* ctx, unsigned char* data, uint32_t data_length,
  ...
  if (!data || !data_length || !packets_per_buffer || !buffers ||
      !max_buffer_size) {
    AUDIO_FUNC_DBG("Invalid parameters passed\n");
    return 0;
  }
  ...
  if (!song->vorbis) {
    ...
    AUDIO_FUNC_DBG("Unable to load vorbis, that sucks VORBIS Error: %i\n",
                   error);
  ...
  if (!song->buffers) {
    ...
    AUDIO_FUNC_DBG("unable to allocate %i bytes for buffer IDs",
...
uint16_t a_song_create_fs(a_ctx* ctx, const char* file_path, const char* name,
  ...
  if (!data) {
    printf("Unable to retrieve data from: %s\n", file_path);
    return 0;
  }
...
uint8_t a_song_destroy(a_ctx* ctx, uint16_t id) {
  if (ctx->song_high < id - 1) {
    AUDIO_FUNC_DBG("no song in context with ID %i\n", id);
    return 0;
  }
...
a_song* a_song_get_open(a_ctx* ctx) {
  if (ctx->song_count == ctx->song_capacity) {
    AUDIO_FUNC_DBG("no open song slots.\n");
    return 0;
  }
...
uint8_t a_song_play(a_ctx* ctx, uint16_t layer_id, uint16_t song_id,
                    a_req* req) {
  if (ctx->song_high < song_id - 1) {
    AUDIO_FUNC_DBG("invalid song ID\n");
    return 0;
  }
  ...
  if (req->fx_count > 0) {
    for (uint16_t i = 0; i < req->fx_count; ++i) {
      ...
      if (req->fx[i] <= ctx->fx_capacity && req->fx[i] > 0) {
        ...
        AUDIO_FUNC_DBG("Invalid fx [%i] passed.\n", req->fx[i]);
      }
    }
  }
  ...
  if (req->filter_count > 0) {
    for (uint16_t i = 0; i < req->filter_count; ++i) {
      ...
      if (req->filters[i] <= ctx->filter_capacity && req->filters[i] > 0) {
        ...
        AUDIO_FUNC_DBG("Invalid filter [%i] passed.\n", req->filters[i]);
      }
    }
  }
#endif
...
uint8_t a_song_stop(a_ctx* ctx, uint16_t song_id) {
  if (ctx->song_high < song_id - 1) {
    AUDIO_FUNC_DBG("no song in slot %i\n", song_id);
    return 0;
  }
...
uint8_t a_song_set_time(a_ctx* ctx, uint16_t song_id, time_s from_start) {
  if (ctx->song_high < song_id - 1) {
    AUDIO_FUNC_DBG("no song in list with id %i\n", song_id);
    return 0;
  }
  ...
  if (approx_sample > song->sample_count) {
    AUDIO_FUNC_DBG("sample requested %i out of range of song %i\n",
...
uint32_t a_song_get_state(a_ctx* ctx, uint16_t song_id) {
  if (ctx->song_high < song_id - 1) {
    AUDIO_FUNC_DBG("no song with ID %i\n", song_id);
    return AL_STOPPED;
  }
...
uint16_t a_song_get_id(a_ctx* ctx, const char* name) {
  if (ctx->song_count == 0) {
    AUDIO_FUNC_DBG("no songs in context.\n");
    return 0;
  }
...
uint16_t a_buf_create(a_ctx* ctx, unsigned char* data, uint32_t data_length,
  ...
  if (!data || !data_length) {
    AUDIO_FUNC_DBG("no asset passed to load into audio buffer.\n");
    return 0;
  }

  if (ctx->buffer_count == ctx->buffer_capacity) {
    AUDIO_FUNC_DBG("no free buffer slots.\n");
    return 0;
  }
  ...
  if (is_ogg) {
    ...
    if (!vorbis) {
      AUDIO_FUNC_DBG("unable to open vorbis header, vorbis error %i\n", error);
      return 0;
    }
    ...
    if (format == -1) {
      AUDIO_FUNC_DBG("Unsupported wave file format.\n");
      return 0;
    }
...
uint8_t a_buf_destroy(a_ctx* ctx, uint16_t buf_id) {
  if (ctx->buffer_high < buf_id - 1) {
    AUDIO_FUNC_DBG("no buffer in slot %i\n", buf_id);
    return 0;
  }
...
a_buf* a_buf_get_id(a_ctx* ctx, uint16_t id) {
  if (ctx->buffer_high < id - 1) {
    AUDIO_FUNC_DBG("id is above the high mark for buffers.\n");
    return 0;
  }
...
a_buf* a_buf_get_open(a_ctx* ctx) {
  if (ctx->buffer_capacity == ctx->buffer_count) {
    AUDIO_FUNC_DBG("no open buffers.\n");
    return 0;
  }
...
uint16_t a_fx_create(a_ctx* ctx, a_fx_type type, void* data) {
  ...
  if (!(type == FX_REVERB || type == FX_EQ)) {
    AUDIO_FUNC_DBG("invalid type of effect passed.\n");
    return 0;
  }

  if (ctx->fx_count == ctx->fx_capacity) {
    AUDIO_FUNC_DBG("no open fx slots.\n");
    return 0;
  }
  ...
  if (!slot) {
    AUDIO_FUNC_DBG("Unable to find open slot for fx\n");
    return 0;
  }
  ...
  switch (type) {
    ...
    case FX_NONE:
      AUDIO_FUNC_DBG("invalid data type.\n");
      return 0;
  }
  ...
  return slot->id;
#else
  AUDIO_FUNC_DBG("AUDIO_AL_NO_FX is defined\n");
  return 0;
#endif
}
...
uint8_t a_fx_destroy(a_ctx* ctx, uint16_t fx_id) {
  ...
  if (fx_id <= 0 || fx_id > ctx->fx_capacity) {
    AUDIO_FUNC_DBG("no fx in that slot.\n");
    return 0;
  }
  ...
  if (!slot) {
    AUDIO_FUNC_DBG("invalid slot pointer.\n");
    return 0;
  }
...
#if !defined(AUDIO_AL_NO_FX)
  if (fx_id <= 0 || fx_id > ctx->fx_capacity) {
    AUDIO_FUNC_DBG("no fx in slot: %i\n", fx_id);
    return 0;
  }
...
uint8_t a_filter_update(a_ctx* ctx, uint16_t filter_id) {
  ...
  if (filter_id > ctx->filter_capacity || filter_id <= 0) {
    AUDIO_FUNC_DBG("no filter in slot %i\n", filter_id);
    return 0;
  }
